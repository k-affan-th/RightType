...
- [~] Native and browser password fields plus a blacklisted terminal pass (matrix: guard_password_field, guard_blacklisted_terminal; manual_selection.py also shows nothing injected into a password input during a focus race). ELECTRON IS NOT COVERED: it is detected through the same UI Automation IsPassword path in focus.rs that the Chromium tests already exercise, so the risk is that a specific Electron app behaves differently, not that the mechanism is unproven. Treat as residual risk, not coverage.
